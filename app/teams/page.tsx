...
export default async function TeamsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <h1 className="text-3xl font-black italic tracking-tight mb-6 text-slate-900 dark:text-white">Teams</h1>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        {(teams ?? []).map((team: Team) => (
          <Link
            key={team.id}
            href={`/teams/${team.slug}`}
            className="bg-white dark:bg-[#111] border border-black/[0.07] dark:border-white/5 rounded-2xl p-6 shadow-sm hover:shadow-lg hover:-translate-y-0.5 hover:border-black/15 dark:hover:border-white/20 transition-all group overflow-hidden relative"
          >
            <div
            ...
            <div className="relative">
              ...
              <div className="mt-4">
                ...
                <p className="text-slate-500 dark:text-[#7a7a7a] text-sm mt-1">{team.university}</p>
                <p className="text-slate-400 dark:text-[#7a7a7a] text-xs mt-2">{countByTeam[team.id] ?? 0} players</p>
              </div>
