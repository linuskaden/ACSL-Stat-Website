...
export default function PlayoffBracket({ games, bracket }: { games: any[]; bracket: any[]; season?: number }) {
  ...
  const champTeam =
    finalGame && finalBe?.winner_id
      ? finalBe.winner_id === finalGame.home_team_id ? finalGame.home_team : finalGame.away_team
      : null
    ...
    return (
      <div className="space-y-6">
        {ROUND_ORDER.filter((r) => byRound[r]?.length).map((round) => (
          <div key={round}>
            <h2 className="text-[10px] font-bold tracking-widest text-slate-400 dark:text-[#555] uppercase mb-3">{round.replace('_', ' ')}</h2>
